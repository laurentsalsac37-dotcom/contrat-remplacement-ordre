...
export function yesNoLabel(input: unknown, emptyLabel = "Non concerné") {
  if (input === "oui" || input === true) return "Oui";
  if (input === "non" || input === false) return "Non";
  if (isEmpty(input)) return emptyLabel;
  return String(input);
}
...
export function modeFacturationLabel(mode?: string) {
  const labels: Record<string, string> = {
    ...
    feuille_soins: "Feuilles de soins papier avec identification du remplaçant",
    feuilles_soins: "Feuilles de soins papier avec identification du remplaçant",
  };
...
function buildSelectedAnnexes(data: Partial<ContractData>) {
  ...
  if (data.annexeJustificatif2400h) {
    annexes.push("Justificatif des 18 mois ou 2 400 heures");
  }

  if (data.annexePlanning) {
    annexes.push("Planning");
  }
...
export function buildContractText(data: Partial<ContractData>) {
  ...
  const showAssociesPhrase = exerciceEnGroupe;
  ...
  const rcpAnnexeText = data.annexeAttestationRcProf
    ? " L’attestation est annexée au présent contrat."
    : "";

  const autorisationCoverageText = autorisationCompatible
    ? " Elle couvre l’intégralité de la période contractuelle."
    : "";
...
Ci-après ${remplaceGrammar.designe} « l’${remplaceGrammar.roleRemplace} »,
...
Pendant les périodes couvertes par le présent contrat, l’infirmier remplacé s’abstient de toute activité infirmière objet du remplacement. Cette obligation ne fait pas obstacle au suivi d’une formation professionnelle, à l’assistance à une personne en péril ni à la participation à un dispositif de secours dans les conditions prévues par les textes applicables.
...
L’infirmier remplaçant atteste disposer d’une assurance responsabilité civile professionnelle valide pour toute la durée du remplacement.${rcpAnnexeText}
...
L’infirmier remplacé exerce-t-il en groupe : ${yesNoLabel(data.exerciceEnGroupe)}.

Type d’exercice en groupe, le cas échéant :
${exerciceEnGroupe ? typeGroupeLabel(data.typeGroupe) : "Non concerné"}

Clause d’agrément prévue par les statuts ou contrats du groupe :
${exerciceEnGroupe ? clauseAgrementLabel(data.clauseAgrement) : "Non concerné"}

Agrément du remplaçant obtenu, le cas échéant :
${exerciceEnGroupe ? yesNoLabel(data.agrementObtenu) : "Non concerné"}

Information des confrères ou associés d’exercice :
${exerciceEnGroupe ? yesNoLabel(data.confreresInformes) : "Non concerné"}

Les patients sont informés dès que possible de la présence, de l’identité et de la qualité de l’infirmier remplaçant : ${yesNoLabel(data.patientsInformes)}.

${showAssociesPhrase ? `Lorsque l’exercice en groupe implique des associés ou des cocontractants, le remplacé confirme avoir informé ces personnes et leur avoir communiqué une copie du présent contrat : ${yesNoLabel(data.associesInformes)}.` : ""}

${exerciceEnGroupe ? "Lorsque l’exercice en groupe impose un agrément, l’infirmier remplacé s’engage à obtenir cet agrément avant le début du remplacement. Même en l’absence d’agrément obligatoire, les parties rappellent l’importance de l’information préalable des confrères au titre de la bonne confraternité." : ""}
...
Lorsque des feuilles de soins papier pré-identifiées au nom de l’infirmier remplacé sont utilisées, le remplaçant doit y faire apparaître visiblement son nom, son prénom, sa qualité d’${remplacantGrammar.roleRemplacant}, ses numéros ordinal et RPPS, ainsi que sa signature.
...
Il sera résilié d’un commun accord entre les parties, sous réserve d’un préavis de ${durationValue(data.preavisCommunAccord)}.
...
Le remplaçant s’interdit tout démarchage de la patientèle du remplacé, toute utilisation irrégulière des fichiers du cabinet et toute manœuvre contraire aux obligations de loyauté et de confraternité.
...
Elles rappellent en particulier que le remplacement est temporaire, que le remplaçant ne remplace pas plus de deux infirmiers simultanément, que le remplacé s’abstient de toute activité infirmière objet du remplacement, sous réserve du suivi d’une formation professionnelle, de l’assistance à une personne en péril et de la participation à un dispositif de secours dans les conditions prévues par les textes applicables, que la durée du remplacement correspond à l’indisponibilité, et qu’un contrat écrit est établi au-delà de vingt-quatre heures ou en cas de remplacement répété.

ARTICLE 20 - SIGNATURES

Fait à ${value(data.faitA)}, le ${value(data.faitLe)}.

Le présent contrat est établi en ${value(data.nombreExemplaires)} exemplaires originaux, dont un pour chacune des parties et un destiné au conseil départemental ou interdépartemental de l’Ordre compétent.

Annexes jointes :
${annexes.length > 0 ? annexes.map((annexe) => `- ${annexe}`).join("\n") : "Aucune annexe sélectionnée."}
